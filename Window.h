...
public:
	// コンストラクタ
	Window(int width = 640, int height = 480, const char* title = "HelloWorld")
		: window(glfwCreateWindow(width, height, title, NULL, NULL)), scale(100.0f), location{ 0.0f, 0.0f }, keyStatus{GLFW_RELEASE}
	{
		if (window == NULL)
		{
			// ウィンドウが作成できなかった
	...
	explicit operator bool()
	{
		...
		if (glfwGetKey(window, GLFW_KEY_LEFT) != GLFW_RELEASE)
			location[0] -= 1.0f / size[0];
		else if (glfwGetKey(window, GLFW_KEY_RIGHT) != GLFW_RELEASE)
			location[0] += 1.0f / size[0];
		if (glfwGetKey(window, GLFW_KEY_DOWN) != GLFW_RELEASE)
			location[1] -= 1.0f / size[1];
